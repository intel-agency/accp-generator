...
- `pr_num`: The PR number to grind review resolutions on.

Repeatedly assign automated reviewers to the PR, then wait for them to conclude their reviews, and then once they finish leavng a batch of review comments, reolsve all of them by invoking the /resolve-pr-comments prompt.
...
Repeat ths entire process (steps 1-3) until there are no more comments left by the automated reviewers in the PR, or the comments that are left are trivial or low priority and can be ignored.
